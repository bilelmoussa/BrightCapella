...
if($product->is_type( 'simple' )) :    
    if ( !empty( $simple_regular_price ) && !empty( $simple_sale_price ) && $simple_regular_price > $simple_sale_price ) : ?>
        <?php echo
            apply_filters( 'woocommerce_sale_flash', '<span class="custom_sale">' . $simple_reduction . '%</span>', $post, $product );
        ?>
<?php endif;
elseif($product->is_type( 'variable' )) :
    if ( !empty( $max_variable_regular_price ) && !empty( $min_variable_sale_price ) && $max_variable_regular_price > $min_variable_sale_price ) : ?>
        <?php echo
            apply_filters( 'woocommerce_sale_flash', '<span class="custom_sale">' . $variable_variable_reduction . '%</span>', $post, $product );
        ?>
    <?php endif;
endif;    
